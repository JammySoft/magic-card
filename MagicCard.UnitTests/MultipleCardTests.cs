...
namespace MagicCard.UnitTests
{
  ...
  public class MultipleCardTests
  {
    ...
    {
      // Create a card
      var card1 = _cardRepository.GetOrAddCard(1000m, "1234", "34567890");
      var card2 = _cardRepository.GetOrAddCard(1000m, "1234", "34567890");
    ...
    public void WithdrawOnLotsOfCardsWithSingleThreadTest()
    {
      ...
      for (int i = 0; i < 100; i++)
      {
        ICard card = _cardRepository.GetOrAddCard(1000m, "1234", "34567890");
    ...
    public void WithdrawOnLotsOfCardsOnMultipleThreadTest()
    {
      ...
      var card1 = _cardRepository.GetOrAddCard(startingBalance, "1234", "34567890");

      // Create 100 other cards.
      var otherCards = Enumerable.Range(1, 100)
        .Select(idx => _cardRepository.GetOrAddCard(startingBalance, "1234", "34567890"))
        .ToArray();
      ...
      Task[] tasks = otherCards.Select(card => Task.Run( () =>
      {
        ...
        this._output.WriteLine($"Withdrew {amountToWithDraw} on thread {Thread.CurrentThread.ManagedThreadId} - balance is {balance}");
